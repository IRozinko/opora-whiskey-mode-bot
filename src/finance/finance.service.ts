...
export class FinanceService {
  ...
  async addExpense(telegramUser: { id: number; username?: string; first_name?: string }, rawText: string) {
    ...
    if (!amount) {
      return 'Не увидел сумму. Формат: /expense 245 happy meal сыну';
    }
    ...
    const categorySum = await this.prisma.transaction.aggregate({
      _sum: { amount: true },
      where: {
        userId: user.id,
        type: TransactionType.expense,
    ...
    const spent = Number(categorySum._sum.amount ?? 0);
  ...
  async addIncome(telegramUser: { id: number; username?: string; first_name?: string }, rawText: string) {
    ...
    if (!amount) {
      return 'Не увидел сумму. Формат: /income 4000 eur вторая работа';
    }
    ...
    await this.prisma.transaction.create({
      data: {
        ...
        amount,
        currency,
        category: description.toLowerCase().includes('чое') ? 'ЧОЕ' : 'Вторая работа',
        description,
      },
    });
  ...
  async moneySummary(telegramUser: { id: number; username?: string; first_name?: string }) {
    ...
    const [income, expenses, debts, goals] = await Promise.all([
      this.prisma.transaction.aggregate({
        _sum: { amount: true },
        where: { userId: user.id, type: TransactionType.income, currency: 'UAH', transactionDate: { gte: monthStart } },
      }),
      this.prisma.transaction.aggregate({
        _sum: { amount: true },
        where: { userId: user.id, type: TransactionType.expense, currency: 'UAH', transactionDate: { gte: monthStart } },
      }),
    ...
    const goalsText = goals
      .map((goal) => `- ${goal.name}: ${Number(goal.currentAmount).toLocaleString('ru-RU')} / ${Number(goal.targetAmount).toLocaleString('ru-RU')} грн (${goal.status})`)
      .join('\n');
